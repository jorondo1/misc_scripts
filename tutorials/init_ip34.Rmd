...
Each command with `>>` will append something to the `.bashrc` file. If you do them multiple times, you'll be adding multiple lines to it.
</div>
...
. ~/.bashrc                                                                        # Executes the .bashrc (happens every time you log in, but we need those variables now!)
```
...
# Going further {.tabset}

## Loading the R environment

If you need extra juice to run a massive DADA2 pipeline your laptop can't handle, for example, you can load the R environment within ip34. First, on ip34, use shared R libraries from the Laforest group:

```{bash r-libs}
echo export R_LIBS="/home/def-ilafores" >> ~/.bashrc
```
